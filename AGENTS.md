...
3. Registers mapping target: `<Plug>(MarkdownLinksAsReferences)` which calls
   `vim_markdown_links_naive#convert()`. Registration is non-overriding:
   existing mapping with the same left-hand side is preserved.
...
2. Startup registration is non-overriding and keeps pre-existing
   `<Plug>(MarkdownLinksAsReferences)` mapping.
